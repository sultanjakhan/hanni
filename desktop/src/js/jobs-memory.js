...
export async function renderJobsMemory(paneEl) {
  ...
  switch (active) {
    ...
    case 'sources': await renderGenericStore(pane, 'jobs_sources', 'Источники', ['TG Python Jobs', 'hh.ru', 'LinkedIn']); break;
...
async function renderPositions(el) {
  ...
  if (roles.length) {
    ...
    html += `<div class="jm-section"><div class="jm-section-title">Роли</div>${cards}</div>`;
  }
...
async function seedPositions() {
  ...
  const roles = [
    ...
    { title: 'Business Analyst', priority: 'высокий' },
  ];
