// ── food-recipe-steps.js — Structured cooking steps ──
const ACTIONS = [
...
export function renderStepsRows(container, ingredientsFn) {
  container.innerHTML = '';
  addStepRow(container, ingredientsFn);
...
function addStepRow(container, ingredientsFn) {
  const addBtn = container.querySelector('.btn-secondary');
  container.insertBefore(createStepRow(ingredientsFn), addBtn || null);
}
...
function createStepRow(ingredientsFn) {
  ...
  // Product dropdown — dynamically populated from ingredients
...
export function collectSteps(container) {
  const steps = [];
