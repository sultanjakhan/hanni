...
export async function renderTableView(el, ctx) {
  ...
  const thCols = unifiedCols.map(col => {
    if (col.kind === 'fixed') {
      ...
      const dt = c.dataType || '';
      if (!displayName) return `<th class="sortable-header fixed-header draggable-col" data-sort="${c.key}" data-fixed-key="${c.key}" data-col-id="${c.key}" data-edit-type="${c.editType || 'text'}" data-data-type="${dt}" style="width:${w}px"></th>`;
      return `<th class="sortable-header fixed-header draggable-col" data-sort="${c.key}" data-fixed-key="${c.key}" data-fixed-label="${escapeHtml(c.label)}" data-col-id="${c.key}" data-edit-type="${c.editType || 'text'}" data-data-type="${dt}" style="width:${w}px"><div class="th-content">${escapeHtml(displayName)}</div></th>`;
    } else {
      const p = col.def;
