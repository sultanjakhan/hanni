...
export async function renderUnifiedLayout(el, tabId, config) {
  ...
  if (config.renderWeights) panes.splice(panes.findIndex(p => p.id === 'goals'), 0, { id: 'weights', icon: '⚖️', label: 'Веса' });
  ...
  switch (activePane) {
    ...
    case 'routine':
      ...
      break;
    case 'weights':
      if (config.renderWeights) await config.renderWeights(paneEl);
      break;
    case 'skills':
    ...
    case 'fridge':
      ...
      break;
    case 'blacklist':
      if (config.renderBlacklist) await config.renderBlacklist(paneEl);
      break;
    case 'picker':
      if (config.renderPicker) await config.renderPicker(paneEl);
      break;
    case 'catalog':
