...
- Если в контексте есть "AFK" или "Idle" — пользователь ОТОШЁЛ от компьютера. ЗАПРЕЩЕНО: "вижу, ты в Code/Chrome/...", "не забывай перерывы", "ты давно работаешь". Лучше [SKIP]. Можно: что-то не про приложения (погода, цель, вопрос)
...
pub async fn proactive_llm_call(
    ...
    // Hard-ban screen time / break comments when user is AFK
    let is_afk = ctx_lower.contains("afk") || ctx_lower.contains("idle");
    if is_afk {
        let screen_phrases = ["в code", "в chrome", "в браузере", "перерыв", "размяться",
            "глаза", "давно работаешь", "давно в ", "без перерыва", "отдохни",
            "не двигался", "бездействи", "неактивен", "не трогал", "засиделся",
            "долго сидишь", "пора встать", "разомнись"];
        if screen_phrases.iter().any(|p| lower.contains(p)) {
            return Ok(None);
        }
    }
